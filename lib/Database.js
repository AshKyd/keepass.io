...
var Database = dejavu.Class.declare({
	...
	loadFile: function(filePath, callback) {
		if(typeof filePath !== 'string'){
			throw new Errors.Argument('Expected `filePath` to be a String');
		}
		if(typeof callback !== 'function'){
			throw new Errors.Argument('Expected `callback` to be a Function');
		}
		...
		fs.readFile(filePath, function(err, content) {
			...
			this.setFile(content, callback);
		}.$bind(this));
	},

	setFile: function(content, callback){
	...
	saveFile: function(filePath, callback) {
		...
		this.getFile(function(err, content){
	...
	getFile: function(callback){
		...
		this._kpDatabase.save(this._credentialStore, callback);
	},
